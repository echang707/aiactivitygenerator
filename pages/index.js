    ...
    >
      <div className="w-full max-w-4xl bg-white rounded-xl shadow-xl p-10 space-y-6 border border-gray-100">
        ...
        <div className="space-y-4">
          <div>
            <label className="block font-bold mb-1">Child&apos;s Age:</label>
            <input
              type="text"
              name="age"
          ...
          {['difficulty', 'category', 'skills', 'learning_style'].map((field) => (
            <div key={field}>
              <label className="block font-bold mb-1 capitalize">{field.replace('_', ' ')}:</label>
              <select
                name={field}
                onChange={handleChange}
                className="w-full border px-3 py-2 rounded"
        ...
        {result && result.type === 'static' && (
          <div className="pt-6 border-t">
            ...
            {result.result['Description'] && (
              <p className="mb-4 text-gray-900">{result.result['Description']}</p>
            )}

            {result.result['Materials Needed'] && (
              <div className="mb-4">
                <p className="font-bold mb-1">Materials Needed:</p>
        ...
        {result && result.type === 'ai' && (
          <div className="pt-6 border-t space-y-6 text-black">
            <h2 className="text-2xl font-bold">AI-Generated Activity</h2>

            {result.result
              .split(/\n(?=\d+\.|Activity|Materials|Instructions|Learning outcomes|Helpful note)/i)
              .filter(Boolean)
              .map((section, i) => {
                const titleMatch = section.match(/^(Activity|Materials|Instructions|Learning outcomes|Helpful note)/i);
                const isTitle = !!titleMatch;

                return (
                  <div key={i} className="text-sm whitespace-pre-line text-gray-900">
                    {isTitle ? (
                      <>
                        <h3 className="text-lg font-semibold mt-4 mb-1">
                          {titleMatch[0].toLowerCase() === 'materials'
                            ? 'Materials Needed'
                            : titleMatch[0].toLowerCase() === 'helpful note'
                            ? 'Helpful Note'
                            : titleMatch[0]}
                        </h3>
                        <p>
